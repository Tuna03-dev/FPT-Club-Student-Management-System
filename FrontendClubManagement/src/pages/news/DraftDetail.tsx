...
import { ArrowLeft, RefreshCw, Send, Pencil, Trash2, Tag, Info } from "lucide-react"
...
export default function DraftDetail() {
  ...
  useEffect(() => {
    ...
    const unsubClub = clubId
      ? subscribeToClub(clubId, (msg) => {
          if (msg.type === "NEWS_DRAFT" &&
            ...
            setInfo("Bản nháp này đã được cập nhật hoặc gửi đi. Quay lại danh sách.")
    ...
    const unsubUser = subscribeToUserQueue((msg) => {
      if (msg.type === "NEWS_DRAFT" &&
        ...
        setInfo("Bản nháp này đã thay đổi hoặc bị xóa.")
        setTimeout(goBack, 2500)
      }
    })
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setInfo("Đã submit bản nháp thành công.")
      setTimeout(goBack, 2000)
    } catch (e: any) {
      setErr(e?.message || "Không submit được.")
    } finally {
      setDoing(null)
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      {/* Thông báo realtime hoặc hành động */}
      {info && (
        ...
        <div className="flex flex-col sm:flex-row gap-2 border-t pt-4">
          ...
          <button
            ...
            className={cn(
              ...
              doing === "submit" ? "bg-indigo-400" : "bg-indigo-600 hover:bg-indigo-700"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "inline-flex items-center gap-2 rounded-lg px-3 py-2 text-white",
              doing === "delete" ? "bg-rose-400" : "bg-rose-600 hover:bg-rose-700"
            )}
          >
