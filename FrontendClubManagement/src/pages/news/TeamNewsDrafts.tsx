...
import { RefreshCw, Clock, ImageOff, Eye, Pencil, Send, Trash2, Info, AlertTriangle } from "lucide-react"
...
export default function TeamNewsDrafts() {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      setInfoBanner(`Đã submit bản nháp #${submitId} lên Chủ nhiệm CLB.`)
      await load()
    } catch (e: any) {
      setErrBanner(e?.message || "Không submit được nháp.")
    } finally {
      setDoing(null)
      setSubmitBusy(false)
      setSubmitId(null)
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setInfoBanner(`Đã xóa bản nháp #${deleteId}.`)
      await load()
    } catch (e: any) {
  ...
  <div className="px-0 sm:px-2 lg:px-4 py-4 space-y-6 w-full">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <button
                        ...
                        title="Xem chi tiết"
                        onClick={() => nav(`/myclub/${clubId}/teams/${teamId}/news/drafts/${d.id}`)}
                      >
      ...
      {/* Submit dialog */}
      <Dialog open={submitId !== null} onOpenChange={(open) => { if (!open) setSubmitId(null) }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Submit bản nháp #{submitId}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-slate-600">Gửi lên Chủ nhiệm CLB để duyệt?</p>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setSubmitId(null)} disabled={submitBusy}>Hủy</Button>
            <Button onClick={confirmSubmit} disabled={submitBusy}>
              {submitBusy ? <RefreshCw className="h-4 w-4 mr-2 animate-spin" /> : null}
              Xác nhận
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deleteId !== null} onOpenChange={(open) => { if (!open) setDeleteId(null) }}>
        <DialogContent>
          ...
          <p className="text-sm text-slate-600">Thao tác này sẽ xóa bản nháp khỏi danh sách.</p>
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setDeleteId(null)} disabled={deleteBusy}>Hủy</Button>
            <Button variant="destructive" onClick={confirmDelete} disabled={deleteBusy}>
              {deleteBusy ? <RefreshCw className="h-4 w-4 mr-2 animate-spin" /> : null}
              Xóa
            </Button>
          </DialogFooter>
        </DialogContent>
