    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-r from-primary to-primary/80 text-primary-foreground py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="text-xl opacity-90">Tìm kiếm và tham gia các câu lạc bộ phù hợp với đam mê của bạn</p>
        </div>
      </div>

      {/* Content */}
      <div className="max-w-6xl mx-auto px-4 py-16">
        {loading && <div className="text-center text-muted-foreground">Đang tải danh sách CLB...</div>}
                ...
                <Link key={club.id} to={`/clubs/${club.id}`}>
                  <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer">
                    <div className="relative h-48 bg-muted overflow-hidden">
                      <img
                        src={club.bannerUrl || club.logoUrl || "/placeholder.svg"}
                        alt={club.clubName}
                        className="w-full h-full object-cover hover:scale-105 transition-transform"
                      />
                    </div>
                    <CardHeader>
                      <CardTitle className="line-clamp-2">{club.clubName}</CardTitle>
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {club.tagsOverflow > 0 && <Badge variant="outline">+{club.tagsOverflow}</Badge>}
                        </div>
