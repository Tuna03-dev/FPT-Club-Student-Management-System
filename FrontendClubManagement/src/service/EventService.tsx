 ...
 */
export async function getStaffAllEvents(params?: {
  startTime?: string;
  endTime?: string;
}): Promise<EventData[]> {
 ...
 */
export async function getStaffEventsByClubId(
  clubId: number,
  params?: { startTime?: string; endTime?: string }
): Promise<EventData[]> {
  const res = await axiosClient.get<EventData[]>(
    `/events/staff/club/${clubId}`,
...
export async function getStaffCancelledEvents(
  clubId?: number
): Promise<EventData[]> {
  const res = await axiosClient.get<EventData[]>(`/events/staff/cancelled`, {
...
export async function cancelClubEventByStaff(
  eventId: number,
  reason?: string
): Promise<void> {
  await axiosClient.post<void>(`/events/${eventId}/cancel`, undefined, {
    params: reason ? { reason } : undefined,
  });
}

export async function restoreCancelledEventByStaff(
  eventId: number
): Promise<void> {
  await axiosClient.post<void>(`/events/${eventId}/restore`);
}

export async function deleteCancelledEventByStaff(
  eventId: number
): Promise<void> {
...
export async function createEvent(
  ...
  if (payload.eventTypeId != null)
    form.append("eventTypeId", String(payload.eventTypeId));
...
export async function updateEvent(
  ...
  if (payload.deleteMediaIds != null && payload.deleteMediaIds.length > 0) {
    payload.deleteMediaIds.forEach((id) =>
      form.append("deleteMediaIds", String(id))
    );
  }
  (payload.images ?? []).forEach((file) => form.append("mediaFiles", file));
...
export async function publishEventByStaff(eventId: number): Promise<EventData> {
  const res = await axiosClient.post<EventData>(
...
export async function getPendingRequests(): Promise<PendingRequestDto[]> {
  const res = await axiosClient.get<PendingRequestDto[]>(
...
export async function getMyDraftEvents(
  clubId?: number
): Promise<MyDraftEventDto[]> {
  const params = clubId ? { clubId } : undefined;
  const res = await axiosClient.get<MyDraftEventDto[]>(
    "/events/my-draft-events",
    { params, timeout: 30000 }
  );
  return res.data ?? [];
}
...
export async function getRegistrationStatus(eventId: number): Promise<boolean> {
  const res = await axiosClient.get<boolean>(
...
export async function getEventRegistrationCount(
  ...
  const res = await axiosClient.get<number>(
    `/events/${eventId}/registration-count`
  );
  return res.data ?? 0;
}
...
export async function getEventRegistrations(
  ...
  const res = await axiosClient.get<EventRegistrationDto[]>(
...
export async function batchMarkAttendance(
  payload: BatchMarkAttendanceRequest
): Promise<void> {
